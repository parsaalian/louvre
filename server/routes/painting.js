...
router.get('/getUserPaintings', auth.isAuthorized, (req, res, next) => {
	...
	rethink
		.queryOfPaintings({ publickey, table: process.env.PAINTING_TABLE })
		.then((result) => {
			successRes(res, "The user's paintings information is sent", result);
		})
		.catch((err) => {
			logger.error(`query on user ${publickey}'s painting information is not responding!: ${err.message}`);
			err.statusCode = 500;
			err.clientCode = 69;
			err.title = 'خطا در سرور';
			err.clientMessage = 'در حال حاضر امکان ارسال نقاشی‌های کاربر  وجود ندارد! لطفا لحظاتی بعد اقدام بفرمایید!';
