...
const awaitDatabase = () => {
    return r
        ...
        .then((tableNames) => {
            ...
            logger.info(
                `Successfully connected to RethinkDB database: ${NAME}`,
            );
        })
        .catch(() => {
...
const connect = () => {
    return r
        ...
        .catch((err) => {
            ...
            logger.error(
                `Unable to connect to RethinkDB with some errors: ${err}`,
            );
        });
};
...
const queryOfUsers = ({ publickey, table }) => {
    ...
    return r
        .table(table)
        .getAll(publickey, { index: "publickey" })
        .run(connection)
        .then((cur) => {
            return cur.toArray((err, result) => {
                if (err) {
                    logger.error(
                        `The queryOfUsers' cursor has some errors : ${err}`,
                    );
                    return null;
    ...
    return req
        ...
        .then((cur) => {
            return cur.toArray((err, result) => {
                if (err) {
                    logger.error(
                        `The queryOfPaintings' cursor has some errors : ${err}`,
                    );
                    return null;
                } else if (!result || result.length === 0) {
                    logger.warn("Thers is no result in queryOfPaintings");
                    return null;
                } else {
                    return JSON.stringify(result, null, 2);
                }
        ...
        .catch((err) => {
            logger.error(`queryOfPaintings is not responding!: ${err.message}`);
        });
};
